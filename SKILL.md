...
**自动验证流程**：
1. 编译PDF：`python D:/教材/通用脚本/compile.py "tex文件"`
2. 验证刚做的题目：`python D:/教材/通用脚本/verify.py "tex文件" --problem X.Y --problem X.Z ...`
3. 验证用DeepSeek API（config.json中已配置）
...
全部题目做完后：
1. 全章编译
2. 全章验证：`python D:/教材/通用脚本/verify.py "tex文件" --all`
3. 汇总报告：✅正确 / ⚠️小问题 / ❌错误
